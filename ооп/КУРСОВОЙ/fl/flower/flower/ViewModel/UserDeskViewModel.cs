...
namespace flower.ViewModel
{
    public class UserDeskViewModel
    {
        ...
        private void Search(object parameter)
        {
            ...
            if (FilterText != null||!string.IsNullOrEmpty(FilterText))
            {
                if (this._userView.ContentControl.ItemsSource == ProductsToShow)
                {
                    ...
                    {
                        if (product.ProductName.Contains(FilterText) || product.ProductDescription.Contains(FilterText))
                        {
                ...
                else if (this._userView.ContentControl.ItemsSource == OrdersToShow)
                {
                    ...
                    {
                        if (order.OrderStatus.Contains(FilterText) || order.TotalPrice.ToString().Contains(FilterText) || order.User.UserName.Contains(FilterText))
                        {
                ...
                else if (this._userView.ContentControl.ItemsSource == UsersToShow)
                {
                    ...
                    {
                        if (user.UserName.Contains(FilterText) || user.UserSurename.Contains(FilterText) || user.Email.Contains(FilterText))
                        {
                ...
                else if (this._userView.ContentControl.ItemsSource == ReviewsToShow)
                {
                    ...
                    {
                        if (review.ReviewText.Contains(FilterText) || review.CreationDate.ToString().Contains(FilterText))
                        {
        ...
        {
            if (App.repository.DeleteOrder(order.OrderId))
            {
