...
namespace DAL
{
    ...
    public class Repository : IRepository
    {
        ...
        public bool  DeleteOrder(int id)
        {
            Order? orderToRemove = this._context.Orders.FirstOrDefault(o => o.OrderId == id);
            if (orderToRemove is not null)
            {

                
                List<Review> reviews = this._context.Reviews.Where(r => r.OrderId == id).ToList();
                if (reviews.Count > 0)
                {
                ...
                this._context.Orders.Remove(orderToRemove);
                try
                {
                    this._context.SaveChanges();
                }
        ...
        public bool UpdOrderReviews(Order order,int id)
        {
            Order?orderToUpd = this._context.Orders.FirstOrDefault(o=>o.OrderId==id);
            if(orderToUpd is not null)
            {
                orderToUpd.Reviews = order.Reviews;
                this._context.Orders.Update(orderToUpd);
                this._context.SaveChanges();
                return true;
            }
            else
        ...
        {
            return this._context.Orders.FirstOrDefault(o=>o.OrderId == id);
        }
        public void Dispose() { }
    }
}
