...
namespace lab12
{
    ...
    public static class AAMLog
    {
        ...
        public static void SearchLogByAction(string action)
        {
            ...
            List<AAMLogRecord> records = new List<AAMLogRecord>();
            records = JsonSerializer.Deserialize<List<AAMLogRecord>>(jsonrecords);
        ...
        public static void SearchByTimeRange(DateTime from, DateTime to)
        {
            ...
            { if (record.Date >= from && record.Date <= to) 
                { Console.WriteLine($"action: {record.Action}, filename: {record.Filename}, filepath: {record.Filepath}, date: {record.Date}"); counter++; }
            }
